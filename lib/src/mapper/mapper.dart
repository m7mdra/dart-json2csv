...
Map<String, dynamic> _createMap(Object object) {
  ...
  var variables = mirror.declarations.values
      .where((it) => it is VariableMirror && !it.isStatic && !it.isPrivate);
  ...
  for (var variable in variables) {
    ...
    var requestedKeyMirror =
        variable.metadata.firstWhere((it) => it.reflectee is Key);

    var name = requestedKeyMirror == null
        ? MirrorSystem.getName(variable.simpleName)
        : (requestedKeyMirror.reflectee as Key).value;
...
bool _isScalar(obj) => obj is String || obj is bool || obj is num;
